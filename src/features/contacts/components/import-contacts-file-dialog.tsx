  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          mappingStep
            ? "flex !h-[min(92dvh,900px)] !w-[min(96vw,1200px)] !max-w-[1200px] flex-col gap-0 overflow-hidden rounded-lg border-border p-0"
            : "!w-[min(92vw,680px)] !max-w-[680px] rounded-lg border-border"
        )}
        showCloseButton
      >
        {mappingStep ? (
        ...
        <DialogFooter
          className={cn(
            "gap-2 sm:gap-0",
            mappingStep &&
              "mx-0 mb-0 mt-0 shrink-0 rounded-none border-t border-border bg-slate-50/80 px-4 py-4 dark:border-slate-800 dark:bg-slate-900/60 sm:px-8"
          )}
        >
          <Button
            type="button"
