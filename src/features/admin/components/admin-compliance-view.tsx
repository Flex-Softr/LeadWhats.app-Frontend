...
import type { AdminComplianceResponse } from "@/types/admin-compliance";
...
type AdminComplianceViewProps = {
  module: AdminModuleDefinition;
};

export function AdminComplianceView({ module }: AdminComplianceViewProps) {
  const { userId, workspaceId, routeKey } = useSessionIdentity();
  const [data, setData] = React.useState<AdminComplianceResponse | null>(null);
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const json = await apiJson<AdminComplianceResponse>("/v1/admin/compliance");
      setData(json);
    } catch (e) {
    ...
    <div className="mx-auto w-full max-w-6xl space-y-8">
      <div className="space-y-1">
        ...
        {data ? (
          <p className="text-xs text-slate-400 dark:text-slate-500">
            ...
            {data.scope === "platform" && data.templates.length >= 200
              ? " · Showing 200 most recently updated templates"
              : null}
            {data.scope === "workspace" && data.templates.length >= 150
              ? " · Showing 150 most recently updated templates"
              : null}
          </p>
        ) : null}
      </div>
      ...
      {loading && !data ? (
        <div className="flex min-h-[30vh] flex-col items-center justify-center gap-3 text-slate-500 dark:text-slate-400">
          ...
          <p className="text-sm">Loading compliance snapshot…</p>
        </div>
      ) : null}
        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card className="rounded-2xl border-slate-200/90 dark:border-slate-800">
              <CardHeader>
                ...
                <CardDescription>
                  Consent / verification quality for audience sends.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {data.contactByStatus.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={2} className="text-muted-foreground">
                          No contacts in scope.
                        </TableCell>
                      </TableRow>
                    ) : (
                      data.contactByStatus.map((r) => (
                        <TableRow key={r.key}>
            ...
            <Card className="rounded-2xl border-slate-200/90 dark:border-slate-800">
              <CardHeader>
                ...
                <CardDescription>
                  High-volume sends (scheduled, completed, failed).
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {data.bulkCampaignByStatus.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={2} className="text-muted-foreground">
                          No bulk campaigns in scope.
                        </TableCell>
                      </TableRow>
                    ) : (
                      data.bulkCampaignByStatus.map((r) => (
                        <TableRow key={r.key}>
                          <TableCell className="font-mono text-xs">
                            {r.key}
                          </TableCell>
          ...
          <Card className="rounded-2xl border-slate-200/90 dark:border-slate-800">
            <CardHeader>
              ...
              <CardDescription>
                Library entries used for template sends and automations. Inactive
                templates stay in the DB but are hidden from pickers.
              </CardDescription>
            </CardHeader>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="hidden sm:table-cell">WA name</TableHead>
                    <TableHead className="hidden md:table-cell">Active</TableHead>
                    <TableHead className="hidden lg:table-cell">Lang</TableHead>
