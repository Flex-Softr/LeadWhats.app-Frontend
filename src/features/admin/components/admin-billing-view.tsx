...
import type { AdminBillingResponse } from "@/types/admin-billing";
...
export function AdminBillingView({ module }: AdminBillingViewProps) {
  const { userId, workspaceId, routeKey } = useSessionIdentity();
  const [data, setData] = React.useState<AdminBillingResponse | null>(null);
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const json = await apiJson<AdminBillingResponse>("/v1/admin/billing");
      setData(json);
    } catch (e) {
      ...
      {data ? (
        <>
          ...
          <Card className="rounded-2xl border-slate-200/90 dark:border-slate-800">
            <CardHeader>
              <CardTitle className="text-base">
                {data.scope === "platform"
                  ? "Revenue-related workspace snapshot"
                  : "Your workspace"}
              </CardTitle>
              <CardDescription>
                Plans and Stripe customer/subscription IDs as stored after
          ...
          {data.scope === "platform" ? (
            <Card className="rounded-2xl border-slate-200/90 dark:border-slate-800">
              <CardHeader>
                <CardTitle className="text-base">Stripe webhooks (recent)</CardTitle>
                <CardDescription>
                  Last processed events from your webhook endpoint (idempotency log).
                  Newest first.
                </CardDescription>
              </CardHeader>
              <CardContent className="overflow-x-auto">
                {data.recentWebhookEvents.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No events recorded yet. Trigger a test event from Stripe or
                    complete a checkout.
                  </p>
                ) : (
                  <Table>
                    ...
                    <TableBody>
                      {data.recentWebhookEvents.map((ev) => (
                        <TableRow key={ev.stripeEventId}>
                          <TableCell className="font-mono text-xs">
                            {ev.type}
                          </TableCell>
                          <TableCell className="hidden font-mono text-xs text-muted-foreground sm:table-cell">
                            {shortId(ev.stripeEventId)}
                          </TableCell>
                          <TableCell className="text-muted-foreground">
                            {fmtDate(ev.processedAt)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
            ...
            <p className="text-sm text-muted-foreground">
              Stripe webhook activity is shown in{" "}
              <span className="font-medium text-slate-700 dark:text-slate-300">
                platform
              </span>{" "}
              scope only (operator email in{" "}
